...
const CINEVERSE_BUILD = "2026.04.28-bulk-import-v21-safe-bulk-quality-audit";
...
function AdminPage({ movies, uploads, addMovie, deleteMovie, syncUploadsToAlgolia, lastAlgoliaSync, adminAlgoliaMessage, setAdminAlgoliaMessage }) {
  ...
  const bulkQualityAuditPreview = bulkQualityAuditItems.slice(0, 12);
          ...
          <div>
            ...
            <div className="changelogBox">
              ...
              {bulkQualityAuditPreview.length > 0 ? (
                <ul>
                  {bulkQualityAuditPreview.map((item) => (
                    <li key={item.id || item.title}>
                      {item.title} — {item.source_type || item.sourceType || "sursă necunoscută"}
                    </li>
                  ))}
                </ul>
              ) : (
