...
const CINEVERSE_BUILD = "2026.04.26-ai-recommendations";
...
function buildAiRecommendations(uploads = []) {
  ...
  uploads.forEach((upload) => {
    ...
    keys.forEach((key) => {
      ...
      groups[cleanKey].count += 1;
      groups[cleanKey].uploads.push(upload);

      if (metadata.genre) groups[cleanKey].genres.add(metadata.genre);
      if (Array.isArray(metadata.tags)) {
        metadata.tags.forEach((tag) => groups[cleanKey].tags.add(tag));
      }
    });
  });
  ...
  return (
    <>
      ...
      <section className="section">
        ...
        <p>Recomandări generate local din upload-urile tale: francize, genuri, colecții și tags.</p>
            ...
            {aiRecommendations.map((item) => (
              <article className="recommendationCard" key={item.label}>
                <div>
                  ...
                  <h3>{item.label}</h3>
                  <p>{item.count} potriviri în biblioteca ta</p>
                  <p>{item.genres.length ? item.genres.join(" · ") : "General"}</p>
                </div>
