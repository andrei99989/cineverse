...
export default function BulkImportPanel() {
  ...
  const handleImportAll = async () => {
    ...
    const okConfirm = window.confirm(`Importi ${items.length} linkuri în Cloudflare D1?`);
    if (!okConfirm) return;
    ...
    for (const item of items) {
      try {
        const payload = {
          ...
          notes: "Bulk import · " + item.sourceType,
    ...
    setImportReport({
      ...
      message: failed ? "Import finalizat cu erori." : "Import finalizat cu succes.",
      results
    });
