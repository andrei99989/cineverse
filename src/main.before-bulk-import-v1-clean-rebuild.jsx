...
const CINEVERSE_BUILD = "2026.04.28-bulk-import-v1-simple-preview";
...
function UploadPage({ uploads, addUpload, deleteUpload, updateUpload, onEdit, onInfo, onPlay }) {
  ...
  const [bulkImportDetectedCount, setBulkImportDetectedCount] = useState(0);
  const [bulkImportDefaultCategory, setBulkImportDefaultCategory] = useState("Filme");
  const [bulkImportDefaultGenre, setBulkImportDefaultGenre] = useState("General");
  const [bulkImportDefaultCountry, setBulkImportDefaultCountry] = useState("All");
  const [bulkImportDefaultLanguage, setBulkImportDefaultLanguage] = useState("All");
  const [bulkImportDefaultQuality, setBulkImportDefaultQuality] = useState("HD");

  const getBulkImportItems = () => {
    const raw = String(bulkImportText || "");

    const iframeMatches = [...raw.matchAll(/<iframe[^>]*src=["']([^"']+)["'][^>]*>/gi)].map((match) => ({
      raw: match[0],
      value: match[1],
      inputType: "iframe"
    }));
    ...
    const urlMatches = [...raw.matchAll(/https?:\/\/[^\s"'<>]+/gi)]
      .map((match) => match[0])
      .filter((url) => !iframeMatches.some((iframe) => iframe.value === url))
      .map((url) => ({
        raw: url,
        value: url,
        inputType: "url"
    ...
    const rumbleMatches = [...raw.matchAll(/Rumble\(["']play["'],\s*\{[^}]*["']video["']\s*:\s*["']([^"']+)["']/gi)]
      .map((match) => ({
    ...
    return [...iframeMatches, ...urlMatches, ...rumbleMatches].map((item, index) => ({
      ...
      value: item.value,
      valid: !!item.value && (item.value.startsWith("http") || item.value.startsWith("//")),
      error: item.value ? "" : "Nu pot extrage URL/iframe src",
      metadata: {
        category: bulkImportDefaultCategory,
        genre: bulkImportDefaultGenre,
        country: bulkImportDefaultCountry,
        language: bulkImportDefaultLanguage,
        quality: bulkImportDefaultQuality,
        tags: ["bulk-import"]
      }
    }));
  };

  const previewBulkImportItems = () => {
    const element = document.getElementById("bulkImportTextarea");
    const sourceText = element?.value || "";
    const urls = [...sourceText.matchAll(/https?:\/\/[^\s"'<>]+/gi)].map((match) => match[0]);

    const uniqueUrls = [...new Set(urls)];

    const parsed = uniqueUrls.map((value, index) => ({
      id: "bulk-preview-" + index,
      index: index + 1,
      raw: value,
      title: "Bulk Import " + String(index + 1).padStart(3, "0"),
      inputType: "url",
      sourceType: detectBulkSource(value),
      value,
      valid: !!value && value.startsWith("http"),
      error: value ? "" : "Nu pot extrage URL",
      metadata: {
        category: bulkImportDefaultCategory,
        genre: bulkImportDefaultGenre,
        country: bulkImportDefaultCountry,
    ...
    setBulkImportText(sourceText);
    setBulkImportPreview(parsed);
    setBulkImportDetectedCount(parsed.length);
  };
  ...
  const clearBulkImport = () => {
    ...
    setBulkImportDetectedCount(0);
  };

  const countBulkImportLinks = () => {
    return (String(bulkImportText || "").match(/https?:\/\/[^\s"'<>]+/gi) || []).length;
  };
            ...
            <div className="bulkImportPanel">
              <div className="bulkImportHeader">
                <div>
                  ...
                  <p className="mutedText">Lipește multe linkuri sau iframe-uri, câte unul pe rând. v1 face doar preview, fără salvare în D1.</p>
                </div>
              </div>
              ...
              <div className="bulkImportDefaults">
                <label>
                  Categorie default
                  <input value={bulkImportDefaultCategory} onChange={(event) => setBulkImportDefaultCategory(event.target.value)} />
                </label>
                <label>
                  Gen default
                  <input value={bulkImportDefaultGenre} onChange={(event) => setBulkImportDefaultGenre(event.target.value)} />
                </label>
                <label>
                  Țară default
                  <input value={bulkImportDefaultCountry} onChange={(event) => setBulkImportDefaultCountry(event.target.value)} />
                </label>
                <label>
                  Limbă default
                  <input value={bulkImportDefaultLanguage} onChange={(event) => setBulkImportDefaultLanguage(event.target.value)} />
                </label>
                <label>
                  Calitate default
                  <input value={bulkImportDefaultQuality} onChange={(event) => setBulkImportDefaultQuality(event.target.value)} />
                </label>
              </div>
              ...
              <textarea
                ...
                onChange={(event) => {
                  ...
                  setBulkImportText(nextText);

                  const raw = String(nextText || "");
                  const urls = [...raw.matchAll(/https?:\/\/[^\s"'<>]+/gi)].map((match) => match[0]);
                  const iframeUrls = [...raw.matchAll(/<iframe[^>]*src=["']([^"']+)["'][^>]*>/gi)].map((match) => match[1]);
                  const rumbleEmbeds = [...raw.matchAll(/Rumble\(["']play["'],\s*\{[^}]*["']video["']\s*:\s*["']([^"']+)["']/gi)].map((match) => "https://rumble.com/embed/" + match[1]);

                  const allValues = [...new Set([...iframeUrls, ...urls, ...rumbleEmbeds])];

                  const parsed = allValues.map((value, index) => ({
                    id: "bulk-preview-" + index,
                    index: index + 1,
                    raw: value,
                    title: "Bulk Import " + String(index + 1).padStart(3, "0"),
                    inputType: iframeUrls.includes(value) ? "iframe" : rumbleEmbeds.includes(value) ? "embed" : "url",
                    sourceType: detectBulkSource(value),
                    value,
                    valid: !!value && (value.startsWith("http") || value.startsWith("//")),
                    error: value ? "" : "Nu pot extrage URL/iframe src",
                    metadata: {
                      category: bulkImportDefaultCategory,
                      genre: bulkImportDefaultGenre,
                      country: bulkImportDefaultCountry,
                ...
                placeholder={"Lipește aici linkuri sau iframe-uri, câte unul pe rând...\nhttps://youtube.com/watch?v=...\n<iframe src=\"https://www.youtube.com/embed/...\"></iframe>"}
              />

              <div className="bulkImportActions">
                <button type="button" className="secondary" onClick={previewBulkImportItems}>Recalculează preview</button>
                <button type="button" className="secondary" onClick={clearBulkImport}>Curăță</button>
                <span className="mutedText">Linkuri găsite text: {bulkImportDetectedCount}</span>
                <span className="mutedText">Detectate: {bulkImportPreview.length}</span>
              </div>

              {bulkImportPreview.length > 0 && (
                <div className="bulkImportPreview">
                  <div className="bulkImportStats">
                    <span className="pill">Total: {bulkImportPreview.length}</span>
                    <span className="pill">Valide: {bulkImportPreview.filter((item) => item.valid).length}</span>
                    <span className="pill">Eșuate: {bulkImportPreview.filter((item) => !item.valid).length}</span>
                  </div>

                  <div className="bulkImportPreviewList">
                    {bulkImportPreview.slice(0, 50).map((item) => (
                      <div className={item.valid ? "bulkImportPreviewItem" : "bulkImportPreviewItem invalid"} key={item.id}>
                        ...
                        {!item.valid && <small>{item.error}</small>}
                      </div>
                    ))}
                  </div>

                  {bulkImportPreview.length > 50 && (
                    <p className="mutedText">Se afișează primele 50 din {bulkImportPreview.length} itemuri.</p>
                  )}
                </div>
              )}
            </div>
