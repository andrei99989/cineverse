...
const CINEVERSE_BUILD = "2026.04.28-ai-library-v25-cloud-presets-ui-polish";
...
function AiLibraryPage({ uploads, onPlay, onEdit, onInfo, onDelete, updateUpload }) {
  ...
  const [savedFilterPresets, setSavedFilterPresets] = useState(() => sortSavedAiLibraryPresets(readSavedAiLibraryPresets()));
  ...
  const clearLocalAiLibraryPresetCache = async () => {
    try {
      localStorage.removeItem(AI_LIBRARY_SAVED_PRESETS_KEY);
      setCopyLinkToast("Cache local preseturi șters.");
  ...
  const loadCloudAiLibraryPresets = async () => {
    ...
    try {
      ...
      writeSavedAiLibraryPresets(items);
    } catch (error) {
  ...
  const deleteSavedAiLibraryPreset = async (id) => {
  ...
  const updateSavedAiLibraryPreset = async (preset) => {
    ...
    writeSavedAiLibraryPresets(sortedNext);
    try {
              ...
              <div className="presetCloudStatus">
                <div className="presetCloudStatusText">
                  ...
                  {!cloudPresetsLoading && !cloudPresetsError && (
                    <span className="mutedText">Preset-urile sunt salvate în Cloudflare D1.</span>
                  )}
                </div>
