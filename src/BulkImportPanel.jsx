...
function buildLocalAiMetadata(upload) {
  ...
  return {
    ...
    country: currentMetadata.country && currentMetadata.country !== "All" ? currentMetadata.country : "Statele Unite",
    language: currentMetadata.language && currentMetadata.language !== "All" ? currentMetadata.language : "Engleză",
    videoQuality: currentMetadata.videoQuality || currentMetadata.quality || "HD",
    quality: currentMetadata.videoQuality || currentMetadata.quality || "HD",
    sourceType,
    ...
    <div className="bulkImportPanel" data-bulk-import-panel="component-v1">
      ...
      {aiQueueReport && (
        <div className="bulkAiQueueReport">
          ...
          {hasAiQueueItems && (
            <button type="button" className="secondary" onClick={handleProcessFirstAiMetadata} disabled={aiQueueLoading}>
              {aiQueueLoading ? "Procesez AI..." : "Procesează primul AI"}
            </button>
          )}
          {aiQueueActionMessage && !isAiQueueComplete && <span className="mutedText">Ultima acțiune: {aiQueueActionMessage}</span>}
          {metadataHandoffMessage && !isAiQueueComplete && <span className="mutedText">Handoff metadata: {metadataHandoffMessage}</span>}
