...
const CINEVERSE_BUILD = "2026.04.26-ai-recommendations-v3-search";
...
function App() {
  ...
  function openMovieSearchFromRecommendation(query) {
    setMovieSearchSeed(query || "");
    setPage("movies");
  }
...
function buildAiRecommendations(uploads = []) {
  ...
  const priorityFields = ["franchise", "collection", "genre"];

  const groups = {};

  function addKey(key, upload, metadata, weight = 1) {
  ...
  uploads.forEach((upload) => {
    ...
    priorityFields.forEach((field) => {
      if (metadata[field]) addKey(metadata[field], upload, metadata, 4);
    });

    if (Array.isArray(metadata.tags)) {
      metadata.tags.forEach((tag) => addKey(tag, upload, metadata, 1));
    }
  });

  return Object.values(groups)
    .sort((a, b) => b.score - a.score || b.count - a.count || a.label.localeCompare(b.label, "ro"))
    .slice(0, 8)
    .map((item) => ({
      ...item,
...
function HomePage({ selectedMovie, setPage, movies, uploads, apiStatus, watchHistory = [], resetWatchHistory, onPlay, onSearchRecommendation }) {
  ...
  return (
    <>
      ...
      <section className="section">
        ...
        <p>Recomandări inteligente generate din francize, colecții, genuri și tags relevante.</p>
          ...
          <div className="recommendationGrid">
            {aiRecommendations.map((item) => (
              <article className="recommendationCard" key={item.label}>
                ...
                <div className="row">
                  <button onClick={() => onSearchRecommendation ? onSearchRecommendation(item.label) : setPage("movies")}>Caută în Filme</button>
                  {item.uploads[0] && (
...
function MoviesPage({ movies, uploads = [], selectedMovie, setSelectedMovie, toggleWatchlist, isSaved, onPlay, onEdit, onInfo, searchSeed = "", clearSearchSeed }) {
