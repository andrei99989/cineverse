...
const CINEVERSE_BUILD = "2026.04.27-ai-library-v11-preset-memory";
...
function AiLibraryPage({ uploads, onPlay, onEdit, onInfo, onDelete, updateUpload }) {
  const [libraryQuery, setLibraryQuery] = useState(() => readAiLibraryFilterMemory().libraryQuery || "");
  const [libraryCategory, setLibraryCategory] = useState(() => readAiLibraryFilterMemory().libraryCategory || "All");
  const [libraryGenre, setLibraryGenre] = useState(() => readAiLibraryFilterMemory().libraryGenre || "All");
  const [librarySource, setLibrarySource] = useState(() => readAiLibraryFilterMemory().librarySource || "All");
  const [librarySort, setLibrarySort] = useState(() => readAiLibraryFilterMemory().librarySort || "newest");
  const [libraryQualityFilter, setLibraryQualityFilter] = useState(() => readAiLibraryFilterMemory().libraryQualityFilter || "All");
  const [libraryCountry, setLibraryCountry] = useState(() => readAiLibraryFilterMemory().libraryCountry || "All");
  const [libraryLanguage, setLibraryLanguage] = useState(() => readAiLibraryFilterMemory().libraryLanguage || "All");
  const [libraryYear, setLibraryYear] = useState(() => readAiLibraryFilterMemory().libraryYear || "All");
  const [libraryVideoQuality, setLibraryVideoQuality] = useState(() => readAiLibraryFilterMemory().libraryVideoQuality || "All");
