...
const CINEVERSE_BUILD = "2026.04.26-ai-library-v5-quality-panel";
...
function AiLibraryPage({ uploads, onPlay, onEdit, onInfo, onDelete, updateUpload }) {
  ...
  const filteredLibraryUploads = uploads
    .filter((upload) => {
      ...
      return queryOk && categoryOk && genreOk && sourceOk;
    })
    .sort((a, b) => {
  ...
  const totalPages = Math.max(1, Math.ceil(filteredLibraryUploads.length / PAGE_SIZE));
  const currentPage = Math.min(page, totalPages);
  const start = (currentPage - 1) * PAGE_SIZE;
  const currentItems = filteredLibraryUploads.slice(start, start + PAGE_SIZE);
  ...
  return (
    <main>
      <section className="section">
          ...
          <div>
            ...
            <div className="toolbar">
              ...
              <select value={libraryGenre} onChange={(event) => setLibraryGenre(event.target.value)}>
                {libraryOptions("genre").map((item) => (
                  <option key={item} value={item}>Gen: {item}</option>
                ))}
              </select>

              <select value={librarySource} onChange={(event) => setLibrarySource(event.target.value)}>
            ...
            <div className="librarySummary">
              <span>Rezultate: {filteredLibraryUploads.length}</span>
            ...
            <div className="details">
              <div>
                ...
                <div className="activeFilterChips">
                  {libraryQuality.withoutPoster > 0 && <span className="filterChip">Fără poster: {libraryQuality.withoutPoster}</span>}
                  {libraryQuality.withoutGenre > 0 && <span className="filterChip">Fără gen: {libraryQuality.withoutGenre}</span>}
                  {libraryQuality.withoutYear > 0 && <span className="filterChip">Fără an: {libraryQuality.withoutYear}</span>}
                  {libraryQuality.withoutTags > 0 && <span className="filterChip">Fără tags: {libraryQuality.withoutTags}</span>}
                </div>
              </div>
            </div>
        ...
        <div className="librarySummary">
          <span>Total upload-uri: {uploads.length}</span>
          <span>Rezultate filtrate: {filteredLibraryUploads.length}</span>
